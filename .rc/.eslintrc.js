module.exports = {
  ...
  rules: {
    'vue/no-template-shadow': 0, // 不允许对在外部范围中声明的变量进行隐藏
    // 一行时最多5个属性，多行时每行最多一个属性
    ...
    'vue/no-parsing-error': [
      2,
      { 'invalid-first-character-of-tag-name': false } // 标签中的特殊字符不检查
    ],
    // name属性使用大写驼峰
    ...
    'vue/html-closing-bracket-newline': [
      2,
      {
        singleline: 'never', // 单行时禁止在右括号前换行
        multiline: 'always' // 多行时在右括号前需要一个换行符
      }
    ],
    'vue/no-side-effects-in-computed-properties': 0, // 不允许在计算属性中出现副作用
    'vue/attribute-hyphenation': 0, // 组件的属性用连字号连接
